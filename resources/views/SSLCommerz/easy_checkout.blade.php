            ...
            <div class="col-12 order-md-1">
                ...
                <form method="POST" class="needs-validation" novalidate action="{{ route('pay.ssl') }}">
                    ...
                    <div class="row">
                        ...
                        <div class="col-6">
                            <div class="mb-3">
                                <label for="pay">Paying amount</label>
                                <input readonly type="text" class="form-control" id="pay" placeholder="" value="BDT {{ $enroll_data['paid'] }}" required>
                            </div>
                        </div>
                    </div>
                    @if ($course->price > $enroll_data['paid'])
                        <p class="text-orange">Your Due will be BDT {{ $course->price - $enroll_data['paid'] }}.</p>
                    @endif

                    @if ($course->price < $enroll_data['paid'])
                        <p class="text-danger">You can't pay more than the fee! Please correct the paying amount.</p>
                    @endif
